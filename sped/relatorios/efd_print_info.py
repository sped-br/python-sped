...
class EFD_Contrib_Info(ArquivoDigital):
	...
	def formatar_cst(codigo_cst):
		try:
			...
			return f'{codigo_cst} - {__class__.tabela_cst[codigo_cst]}'
		except:
			return codigo_cst
	...
	def formatar_nbc(natureza_bc):
		try:
			...
			return f'{natureza_bc} - {__class__.tabela_bc_do_credito[natureza_bc]}'
		except:
			return natureza_bc
	...
	def formatar_tipo(tipo_do_item):
		try:
			...
			return f'{tipo_do_item} - {__class__.tabela_tipo_do_item[tipo_do_item]}'
		except:
			return tipo_do_item
	
	def formatar_mod(doc_fiscal):
		try:
			return f'{doc_fiscal} - {__class__.tabela_modelos_documentos_fiscais[doc_fiscal]}'
		except:
			return doc_fiscal
		...
		if codigo_da_conta != '':
			for campo in self.info_da_conta[codigo_da_conta]:
				...
				if campo == 'COD_NAT_CC' and re.search('\d{1,2}', val):
					...
					val = val + ' - ' + __class__.tabela_natureza_da_conta[val]
				dict_info[campo] = val
